...
from cglue.utils.multiple_instance_helpers import (
    add_instance_check,
...
class StructType(TypeCategory):
    ...
    def __init__(self, type_collection):
        attributes = {
            ...
            "static": {"type": BuiltinTypes.STRUCT},
        }
    ...
    def render_typedef(self, type_name, type_data):
        return chevron.render(
            **{
                ...
                "data": {
                    "type_name": type_name,
                    "fields": dict_to_chevron_list(
                        type_data["fields"], key_name="name", value_name="type"
                    ),
                },
            }
        )
    ...
    def render_value(self, type_name, type_data, value, context="assignment"):
        ...
        fields = (
            f'.{rpad(name, length)} = {types.get(t).render_value(value.get(name), "initialization")}'
    ...
    def attribute(self, type_name, type_data: dict, attr_name):
        if attr_name == "default_value":
            ...
            return {
                name: default.get(name) or types.get(field_type).default_value()
    ...
    def render_typedef(self, type_name, type_data):
        return chevron.render(
            **{
                ...
                "data": {
                    "type_name": type_name,
                    "values": chevron_list_mark_last(
                        [{"value": value} for value in type_data["values"]]
                    ),
                },
            }
        )
...
class UnionType(TypeCategory):
    ...
    def __init__(self, type_collection):
        attributes = {
            ...
            "static": {"type": BuiltinTypes.UNION},
        }
    ...
    def render_typedef(self, type_name, type_data):
        return chevron.render(
            **{
                ...
                "data": {
                    "type_name": type_name,
                    "members": dict_to_chevron_list(
                        type_data["members"], key_name="name", value_name="type"
                    ),
                },
            }
        )
    ...
    def render_value(self, type_name, type_data, value, context="assignment"):
        ...
        members = {
            name: self._type_collection.get(member_type)
            for name, member_type in type_data["members"].items()
        }
        values_str = ",\n".join(
            f'.{name} = {members[name].render_value(value, "initialization")}'
...
def lookup_member(types: TypeCollection, data_type, member_list):
    keys = {BuiltinTypes.STRUCT: "fields", BuiltinTypes.UNION: "members"}
...
def process_member_access(
    types: TypeCollection, attributes, provided_data_type, consumed_data_type
):
...
class VariableSignal(SignalType):
    def __init__(self):
        super().__init__(
            consumers="multiple",
            attributes={"optional": {"init_value": None, "member": None}},
        )
    ...
    def generate_provider(
        ...
        if instance_argument is not None:
            ...
            assignment = add_instance_check(
                assignment, provider_instance, instance_arg_name=instance_argument
            )

        return {
            provider_port_name: {
                "write": {"used_arguments": used_args, "body": assignment}
            }
        }
    ...
    def generate_consumer(
        ...
        if instance_argument is not None:
            used_args.append(instance_argument)
            read = add_instance_check(
                read, consumer_instance, instance_arg_name=instance_argument
            )
...
class ArraySignal(SignalType):
    ...
    def create(self, context: RuntimeGeneratorContext, connection: SignalConnection):
        ...
        if type(init_values) is list:
            if len(init_values) != count:
                raise Exception(
                    f"Array initializer count ({len(init_values)}) does not "
        ...
        context["declarations"].append(
            f"static {data_type.name} {connection.name}[{count}] = {{{init_values}}};"
        )
    ...
    def generate_provider(
        ...
        return {
            provider_port_name: {"write": {"used_arguments": used_args, "body": body}}
        }
        ...
        if "count" not in consumer_port_data:
            ...
            try:
                ...
                raise Exception(
                    ...
                    f"array without specifying the element"
                ) from e
        else:
...
class QueueSignal(SignalType):
    def __init__(self):
        super().__init__(
            consumers="multiple",
            attributes={"required": {"queue_length"}, "optional": {"member": None}},
        )
    ...
    def generate_provider(
        ...
        if instance_arg_name:
            ...
            body = add_instance_check(
                body, provider_instance, instance_arg_name=instance_arg_name
            )
    ...
    def generate_consumer(
        ...
        if instance_arg_name:
            used_args.append(instance_arg_name)
            read = add_instance_check(
                read, consumer_instance, instance_arg_name=instance_arg_name
            )
...
class ConstantSignal(SignalType):
    def __init__(self):
        super().__init__(
            consumers="multiple", attributes={"optional": {"member": None}}
        )
    ...
    def generate_consumer(
        ...
        if provider_instance_arg_name:
            ...
            body = add_instance_check(
                body, consumer_instance, instance_arg_name=provider_instance_arg_name
            )
...
class ConstantArraySignal(SignalType):
    def __init__(self):
        super().__init__(
            consumers="multiple",
            attributes={"optional": {"member": None, "index": None}},
        )
...
class ReadValuePortType(PortType):
    ...
    def declare_functions(self, port: Port) -> dict[str, FunctionPrototype]:
        ...
        if data_type.passed_by() == TypeCollection.PASS_BY_VALUE:
            ...
            function = port.declare_function(
                fn_name, "void", {"value": {"direction": "out", "data_type": data_type}}
            )
...
class ReadQueuedValuePortType(PortType):
    ...
    def create_component_functions(
        ...
        function.attributes.add("weak")
        function.set_return_statement(
            self._types.get("QueueStatus_t").render_value(None)
        )
...
class ReadIndexedValuePortType(PortType):
    def __init__(self, types):
        super().__init__(
            ...
            {
                "order": 3,
                "consumes": {"array": "multiple", "constant_array": "multiple"},
                "def_attributes": {
        ...
        if data_type.passed_by() == TypeCollection.PASS_BY_VALUE:
            function = port.declare_function(
                ...
                data_type.name,
                {
                    "index": {
                        "direction": "in",
                        "data_type": self._types.get("uint32_t"),
                    }
                },
            )
        else:
...
class WriteDataPortType(PortType):
    ...
    def declare_functions(self, port: Port) -> dict[str, FunctionPrototype]:
        ...
        function = port.declare_function(
            fn_name, "void", {"value": {"direction": "in", "data_type": data_type}}
        )
...
def process_component_ports_and_types(owner: CGlue, component: Component):
    ...
    try:
        ...
        if component.config["multiple_instances"]:
            ...
            add_type_def(
                owner.types, component.instance_type, instance_type, component.name
            )
