...
class CGlue:
    ...
    def load(self, output_folder=None):
        ...
        for plugin_name in self.settings["required_plugins"]:
            if plugin_name not in self._plugins:
                raise Exception(
                    f"Project requires {plugin_name} plugin, which is not loaded"
                )
        ...
        for type_obj in type_objects:
            if type_obj not in visited_types:
                try:
                    ...
                    yield from self._sort_types_by_dependency(
                        dependencies, visited_types
                    )
    ...
    def update_component(self, component_name: str) -> bool | dict[str, str]:
        ...
        port_short_names = (
            f"{component_name}/{port_name}"
            for port_name in component_object.config["ports"]
        )
        ...
        context = {
            ...
            "declarations": [],
            "header_declarations": [],
            "files": {config_file: "", source_file: "", header_file: ""},
            "folders": [component_name],
            "functions": {
                short_name: self._ports[short_name].create_component_functions()
        ...
        includes = {f'"{component_name}.h"', '"utils.h"'}
        ...
        sorted_type_objects = list(
            self._sort_types_by_dependency(self.types.get(type_names))
        )
    ...
    def get_project_structure(self):
        ...
        port_functions = {
            name: port.create_runtime_functions() for name, port in self._ports.items()
        }
        self._functions.update(port_functions)

        for connection in self._project_config["runtime"]["port_connections"]:
            provider_name, port, attributes, all_signals = self._process_provider_port(
                context, connection
            )
    ...
    def generate_runtime(self) -> dict[str, str]:
        ...
        instance_variables = [
            f"static {instance.component.instance_type} {instance.instance_var_name};"
    ...
    def _create_component_instances(self, context: RuntimeGeneratorContext):
        for component in self._components:
            if not component.config["multiple_instances"]:
                context.component_instances.add(
                    component.create_instance(component.name)
                )

        for instance_name, component_name in self._project_config.get(
            "instances", {}
        ).items():
    ...
    def _process_consumer_ports(
        ...
        inferred_signal_type = provider_port.port_type["provides"].intersection(
        ...
        if len(inferred_signal_type) == 1:
            ...
            raise Exception(
                f"Incompatible ports: {provider_port.full_name} and {consumer_port.full_name}"
            )
        else:
            raise Exception(
                "Connection type can't be inferred for "
        ...
        def create_new_signal(new_signal_name):
            signals_of_current_type.append(
                signal_type.create_connection(
                    context, new_signal_name, provider_name, attrs
                )
            )
        ...
        if not signals_of_current_type:
            ...
            if signal_type.consumers == "multiple_signals":
                ...
                raise Exception(
                    f"Multiple consumers not allowed for {signal_type_name}"
                    f" signal (provided by {provider_port.full_name})"
                )

        consumer_attributes = consumer_ref.get("attributes", {})
        signals_of_current_type[-1].add_consumer(
