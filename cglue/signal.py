...
class SignalConnection:
    def __init__(
        ...
        try:
            ...
            raise Exception(
                f"Failed to generate signal implementation for {self.name}"
            ) from e
    ...
    def add_consumer(self, consumer_name, consumer_attributes):
        self.consumers.append(
            (
                consumer_name,
                self.signal.process_attributes(self.attributes, consumer_attributes),
            )
        )
    ...
    def generate(self):
        ...
        try:
            function_mods_list = [
                self.signal.generate_provider(self.context, self, self.provider)
            ]
        except Exception as e:
            raise Exception(
                f"Failed to generate provider implementation for {self.name}"
            ) from e

        for consumer, attributes in self.consumers:
            try:
                function_mods = self.signal.generate_consumer(
                    self.context, self, consumer, attributes
                )
                function_mods_list.append(function_mods)
            except Exception as e:
                raise Exception(
                    f"Failed to generate consumer implementation for "
                    f"{self.name} (consumer: {consumer})"
                ) from e

        self.context["runtime"].raise_event(
            "signal_generated", self, function_mods_list
        )
...
class SignalType:
    ...
    def create_connection(
        self, context: RuntimeGeneratorContext, name, provider, attributes
    ):
        return SignalConnection(
            context, name, self, provider, self._data_processor.process(attributes)
        )
