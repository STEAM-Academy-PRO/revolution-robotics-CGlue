...
def collect_arguments(
    attributes, consumer_name, consumer_arguments, caller_args, manual_args=None
):
    ...
    for arg in user_arguments:
        if arg not in consumer_arguments:
            print(
                f"Warning: Runnable {consumer_name} does not have an argument named '{arg}'"
            )
    ...
    for arg_name, arg_type in consumer_arguments.items():
        if arg_name in user_arguments:
            ...
            if arg_type != caller_args[arg_name]:
                raise Exception(
                    f"Caller of {consumer_name} has matching argument {arg_name} but types are different"
                )
...
class EventSignal(SignalType):
    def __init__(self):
        super().__init__(
            consumers="multiple", attributes={"optional": {"arguments": {}}}
        )
    ...
    def generate_consumer(
        ...
        if callee_instance_arg_name:
            manual_args[callee_instance_arg_name] = (
                "&" + consumer_instance.instance_var_name
            )

        caller_args = caller_fn.arguments.copy()
        passed_arguments = collect_arguments(
            attributes,
            consumer_instance_name,
        ...
        instance_arg_name = get_instance_argument(
            list(caller_args.keys()), provider_instance
        )
        ...
        return {
            provider_port_name: {
                "run": {"body": body, "used_arguments": used_args},
...
class ServerCallSignal(SignalType):
    def __init__(self):
        super().__init__(
            consumers="multiple",
            attributes={"optional": {"conditions": {}, "arguments": {}}},
        )
    ...
    def generate_consumer(
        ...
        if provider_instance_arg_name:
            manual_args[provider_instance_arg_name] = (
                "&" + provider_instance.instance_var_name
            )
        ...
        if attributes["conditions"]:
            ...
            conditions = (
                f"{arg} == {value}" for arg, value in attributes["conditions"].items()
            )
        ...
        if instance_arg_name:
            body = add_instance_check(
                body, consumer_instance, instance_arg_name=instance_arg_name
            )
...
def remove_runtime_component(owner: CGlue, config):
    ...
    config["runtime"]["port_connections"] = list(
        connection
