...
class AsyncServerCallSignal(SignalType):
    def __init__(self):
        super().__init__("single", {"required": {"update_on"}, "optional": {}})
    ...
    def create(self, context: RuntimeGeneratorContext, connection: SignalConnection):
        ...
        update_function = FunctionImplementation(
            FunctionPrototype(connection.name + "_Update", "void")
        )
        ...
        if port_component_is_instanced(provider_instance):
            callee_arguments["instance"] = (
                arg_prefix + "&" + provider_instance.instance_var_name
            )
        ...
        for name, arg_data in port_args.items():
            ...
            if (
                arg_dir == "out"
                or arg_dir == "inout"
                or arg_type.passed_by() == TypeCollection.PASS_BY_POINTER
            ):
                callee_arguments[name] = (
                    f"{arg_prefix}&{connection.name}_argument_{name}"
                )
            else:
                callee_arguments[name] = (
                    f"{arg_prefix}{connection.name}_argument_{name}"
        ...
        if "run" in port.functions:
            self._generate_sync_updater_impl(
                callee_arguments, connection, port, update_function
            )

        elif "async_run" in port.functions:
            self._generate_async_updater_impl(
                callee_arguments, connection, port, update_function
            )
        else:
            raise NotImplementedError

    def _generate_async_updater_impl(
        self, callee_arguments, connection, port, update_function
    ):
        lock, unlock = self._get_lock_impl(connection)

        # long running handler function
        update_function.add_body(
            chevron.render(
                **{
                    "template": """{{ lock }}
...
""",
    ...
    def _generate_sync_updater_impl(
        self, callee_arguments, connection, port, update_function
    ):
        lock, unlock = self._get_lock_impl(connection)
...
switch (command)
{
    ...
    def generate_consumer(
        ...
        call_mods = self.generate_call_function(
            ...
            call_function.prototype.arguments,
            connection,
            consumer_instance_name,
            connection.provider,
            provider_port["arguments"],
            lock,
            unlock,
        )
        ...
        get_result_mods = self.generate_get_result_function(
            call_function, connection, result_arg_names, lock, unlock
        )
        ...
        update_call_mods = {"body": update_function.prototype.generate_call({}) + ";"}

        if consumer_is_multiple_instance:
            cancel_mods["used_arguments"].append("instance")
            get_result_mods["used_arguments"].append("instance")

            busy_value = context.types.get("AsyncOperationState_t").render_value(
                "AsyncOperationState_Busy"
            )

            cancel_mods["body"] = add_instance_check(
                cancel_mods["body"], consumer_instance
            )
            call_mods["body"] = add_instance_check(call_mods["body"], consumer_instance)
            call_mods["return_statement"] = busy_value
            get_result_mods["body"] = add_instance_check(
                get_result_mods["body"], consumer_instance
            )
            get_result_mods["return_statement"] = busy_value

        return {
            connection.attributes["update_on"]: {"run": update_call_mods},
            consumer_port_name: {
                "cancel": cancel_mods,
    ...
    @staticmethod
    def generate_get_result_function(
        call_function, connection, result_arg_names, lock, unlock
    ):
        # get result
        ...
        for arg_name in result_arg_names:
            if arg_name not in call_function.arguments:
                ...
                value = call_function.arguments[arg_name]["data_type"].render_value(
                    None
                )
...
return returned_state;""",
        ...
        return {"body": result_body, "used_arguments": used_arguments}
    ...
    def generate_call_function(
        ...
        if extra_args:
            print(
                f'Warning: extra argument "{", ".join(extra_args)}" on signal'
                f" {provider_instance_name}, consumed by {consumer_instance_name}"
            )

        call_arguments, missing_arguments = (
            AsyncServerCallSignal._get_consumer_call_args(
        ...
        if missing_arguments:
            raise Exception(
                f"{consumer_instance_name} does not provide {provider_instance_name} with"
...
return returned_state;""",
        ...
        return {"body": call_body, "used_arguments": call_function_args}

    @staticmethod
    def _get_consumer_call_args(
        signal_arguments, function_args, provider_port_args, types
    ):
        call_arguments = []
                        ...
                        call_arguments.append(
                            {
                                "name": arg,
                                "value": config_value,
                                "by_pointer": arg_type.passed_by()
                                == TypeCollection.PASS_BY_POINTER,
                            }
                        )
                    else:
                        # constant value
    ...
    def _get_lock_impl(connection):
        if (
            "no_locks" not in connection.attributes
            or not connection.attributes["no_locks"]
        ):
    ...
    def __init__(self, types):
        super().__init__(
            types,
            {
                ...
                "def_attributes": {"optional": {"arguments": {}}, "static": {}},
            },
        )
    ...
    def declare_functions(self, port):
        ...
        for name, arg_data in port["arguments"].items():
            if type(arg_data) is str:
                ...
                call_function.arguments.add(
                    name, "in", self._types.get(arg_data["data_type"])
                )
            else:
                result_function.arguments.add(
                    name, "out", self._types.get(arg_data["data_type"])
                )
...
def init(owner: CGlue):
    ...
    types.add(
        ...
        types.category("enum").process_type(
            {
                "values": ["AsyncResult_Pending", "AsyncResult_Ok"],
