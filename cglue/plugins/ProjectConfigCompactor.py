...
def process_port_ref_shorthand(port, port_key="port"):
    ...
    if type(port) is str:
        parts = port.split("/")
        port = {"short_name": port, "component": parts[0], port_key: parts[1]}
    else:
...
def expand_port(runnable, port_key):
    ...
    return {**process_port_ref_shorthand(runnable, port_key), "attributes": attributes}
...
def expand_port_connection(port_connection):
    ...
    attrs = {
        k: v
        for k, v in port_connection.items()
        if k not in ("provider", "consumer", "consumers")
    }
...
def _compact_ref(ref):
    ...
    if set(ref.keys()).difference(
        ("short_name", "component", "port", "runnable", "attributes")
    ):
...
def compact_project_config(owner: CGlue, config):
    ...
    component_types = list(
        component_data.config.get("types", {}) for component_data in owner._components
    )
    ...
    compacted_runtime = {"runnables": {}, "port_connections": []}
    ...
    for connection in expanded_runtime["port_connections"]:
        compacted_connection = {"provider": _compact_ref(connection["provider"])}
        ...
        compacted_connection.update(
            {
                ...
                if key not in ("provider", "consumers")
            }
        )
